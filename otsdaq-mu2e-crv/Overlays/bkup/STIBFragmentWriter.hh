    ...
    : STIBFragment (f), artdaq_Fragment_ (f)
{
	if (!f.hasMetadata () || f.dataSizeBytes () > 0) {
		throw cet::exception ("Error in STIBFragmentWriter: Raw artdaq::Fragment object does not appear to consist of (and only of) its own header + the STIBFragment::Metadata object");
	}
