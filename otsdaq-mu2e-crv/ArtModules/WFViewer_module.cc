...
ots::WFViewer::WFViewer(fhicl::ParameterSet const &ps)
    : art::EDAnalyzer(ps), prescale_(ps.get<int>("prescale")), current_run_(0),
      num_x_plots_(ps.get<std::size_t>(
          "num_x_plots", std::numeric_limits<std::size_t>::max())),
      num_y_plots_(ps.get<std::size_t>(
          "num_y_plots", std::numeric_limits<std::size_t>::max())),
      fragment_type_labels_(
          ps.get<std::vector<std::string>>("fragment_type_labels")),
      fragment_ids_(
          ps.get<std::vector<artdaq::Fragment::fragment_id_t>>("fragment_ids")),
      graphs_(fragment_ids_.size()), histograms_(fragment_ids_.size()),
...
void ots::WFViewer::analyze(art::Event const &e) {
  std::cout << __COUT_HDR_FL__ << "WFViewer Analyzing event " << e.event()
            << std::endl;
  ...
  for (const auto &frag : fragments) {
    ...
    if (!histograms_[ind]) {
      histograms_[ind] = std::unique_ptr<TH1D>(new TH1D(
          Form("Fragment_%d_hist", fragment_id), "Title Dennis", 300,
          (Double_t)0.0, (Double_t)std::numeric_limits<uint8_t>::max()));
