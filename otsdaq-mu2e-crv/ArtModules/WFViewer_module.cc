...
ots::WFViewer::WFViewer (fhicl::ParameterSet const &ps)
    : art::EDAnalyzer (ps), prescale_ (ps.get<int> ("prescale")), current_run_ (0), num_x_plots_ (ps.get<std::size_t> ("num_x_plots", std::numeric_limits<std::size_t>::max ())), num_y_plots_ (ps.get<std::size_t> ("num_y_plots", std::numeric_limits<std::size_t>::max ())), fragment_type_labels_ (ps.get<std::vector<std::string>> ("fragment_type_labels")), fragment_ids_ (ps.get<std::vector<artdaq::Fragment::fragment_id_t>> ("fragment_ids")), graphs_ (fragment_ids_.size ()), histograms_ (fragment_ids_.size ()), outputFileName_ (ps.get<std::string> ("fileName", "otsdaqdemo_onmon.root")), writeOutput_ (ps.get<bool> ("write_to_file", false))
{
...
double ots::WFViewer::calcmean (const float *data)
{
	...
	mean /= 10;
	std::cout << "WFViewer mean is " << mean << " data[0] is " << data[0] << " data[1] is " << data[1] << " 2=" << data[2] << " 3=" << data[3] << std::endl
	          << std::endl;
	std::cout << "WFViewer mean is " << mean << " data[15997] is " << data[15997] << " data[15998] is " << data[15998] << " 2=" << data[15999] << " 3=" << data[15999] << std::endl
	          << std::endl;
