...
#define PROGRAM_FILE_PATH                                                      \
  std::string(getenv("OTS_FIRMWARE_PROGRAM_FILE_PATH")) + "/"
...
FEOtsEthernetProgramInterface::FEOtsEthernetProgramInterface(
  ...
  registerFEMacroFunction(
      ...
      static_cast<FEVInterface::frontEndMacroFunction_t>(
          &FEOtsEthernetProgramInterface::
              getListOfProgramFiles) /*feMacroFunction*/,
...
//========================================================================================================================
// void FEOtsEthernetProgramInterface::configureDetector(const DACStream&
// theDACStream)
//{
...
//	0 args in
//	1 args out
//		listOfProgramFiles = comma-separated list of programmable file
//names
//
// Note: path is from environment variable OTS_FIRMWARE_PROGRAM_FILE_PATH
void FEOtsEthernetProgramInterface::getListOfProgramFiles(
  ...
  auto &returnString =
      FEVInterface::getFEMacroOutputArgument(argsOut, "listOfProgramFiles");
  returnString = "";
...
//		listOfProgramFiles = comma-separated list of programmable file
//names
void FEOtsEthernetProgramInterface::loadProgramFile(
    frontEndMacroInArgs_t argsIn, frontEndMacroOutArgs_t argsOut) {
  // Steps:
  //	- open file
  //
  ...
  // keep as placeholder for potentially handling mcs files (in addition to bin
  // files)
  /*if (path.Find(".mcs") != -1) {
          //if the path has .mcs in it assume it is an mcs file and generate a
  bin file FILE * binfile = fopen("generated_from_mcs.bin", "w"); char
  line[100]; char lineoutput[16]; fgets(line, 100, bitstream);//skip first line
          int i;
          bool has_d;
          //char dstr[100];
          while (fgets(line, 100, bitstream)) {
                  ...
                  has_d = 0;
                  while (i < 41) {
                          lineoutput[(i - 9) / 2] = hextoint(line[i]) * 16 +
  hextoint(line[i + 1]); if (hextoint(line[i + 1]) == 13) has_d = 1; i += 2;
                  }
                  if (has_d) {
                          LineOut(line);
                          break;
          ...
          fclose(bitstream);
          fclose(binfile);
          //update the bitsream variable for the rest of the function to the
  .bin file bitstream = fopen("generated_from_mcs.bin", "r");
  }*/
  ...
  // setup address and flash write mode
  {
    std::vector<uint64_t> dataVec = {
        0, // first (ie at addr 0) quadwrod all 0 because it is the
    ...
    sendBuffer.resize(0);
    OtsUDPFirmwareCore::writeAdvanced(sendBuffer,
                                      UDP_CORE_BLOCK_ADDRESS /*block*/ |
                                          FLASH_COMMAND_BASE /*addr*/,
                                      dataVec /*data*/);
    OtsUDPHardware::write(sendBuffer);
  }
  ...
  while (1) {
    ...
    if (result !=
        page_size) // if block of data is not full page size, handle it.
    {
      if (result == 0) {
    ...
    if (page > 1) // if 3rd page and beyond, handle differently
    {
      ...
      // check status
      sendBuffer.resize(0);
      OtsUDPFirmwareCore::read(sendBuffer, UDP_CORE_BLOCK_ADDRESS /*block*/ |
                                               FLASH_WRITE_STATUS /*addr*/);
      ...
      //			returnaddr = 0 |
      //(int)next_page_recv_buff[21]<<16 | (int)next_page_recv_buff[20]<<8 |
      //(int)next_page_recv_buff[19];
      //			thisbytesoff = ftell(bitstream) - returnaddr;
      //			if (thisbytesoff != bytesoff) {
      //				sprintf(str, "New offset: %X",
      //thisbytesoff);
      //				__COUT__ << str << std::endl;
    ...
    OtsUDPFirmwareCore::writeAdvanced(
        ...
        page_data /*data*/, page_size / 8 /*size in quadwords*/,
    ...
    //ftell(bitstream)-0x200+counter); 			while (1) {
    //				fprintf(outfile, "%2.2hhX ",
