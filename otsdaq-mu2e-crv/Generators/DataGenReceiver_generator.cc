...
void ots::DataGenReceiver::ProcessData_(artdaq::FragmentPtrs& frags)
{
	...
	memcpy((void*)&value, (void*)buffer_.substr(2).data(),
	       8);  // make data counter
	std::cout << __COUT_HDR_FL__ << __PRETTY_FUNCTION__ << std::hex << value << std::dec
	          << std::endl;
	...
	frags.emplace_back(artdaq::Fragment::FragmentBytes(initial_payload_size,
	                                                   ev_counter(),
	                                                   fragment_id(),
	                                                   ots::detail::FragmentType::DataGen,
	                                                   metadata));
	ev_counter_inc();
	...
	std::cout << __COUT_HDR_FL__ << "DJN2 after, datasizebytes=" << thisFrag.asksize()
	          << std::endl
	          << std::endl;
