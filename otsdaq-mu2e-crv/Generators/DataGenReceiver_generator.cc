...
ots::DataGenReceiver::DataGenReceiver (fhicl::ParameterSet const &ps)
    ...
    , DataConsumer (ps.get<std::string> ("SupervisorApplicationUID", "ARTDAQDataManager"), ps.get<std::string> ("BufferUID", "ARTDAQBuffer"), ps.get<std::string> ("ProcessorUID", "DataGenReceiver"), HighConsumerPriority)
...
void ots::DataGenReceiver::ProcessData_ (artdaq::FragmentPtrs &frags)
{
	...
	std::cout << __COUT_HDR_FL__ << "DJN2 after, datasizebytes=" << thisFrag.asksize () << std::endl
	          << std::endl;
