...
ots::DataGenReceiver::DataGenReceiver(fhicl::ParameterSet const &ps)
    : WorkLoop("DataGenReceiver"),
      DataConsumer(
          ps.get<std::string>("SupervisorApplicationUID", "ARTDAQDataManager"),
          ps.get<std::string>("BufferUID", "ARTDAQBuffer"),
          ps.get<std::string>("ProcessorUID", "DataGenReceiver"),
      ...
      ip_(ps.get<std::string>("ip", "127.0.0.1")), expectedPacketNumber_(0),
  ...
  TLOG(TLVL_INFO, "DataGenReceiver")
      << "MY TRIGGER MODE IS: "
      << ps.get<std::string>("trigger_mode", "UNDEFINED!!!");
  std::cout << __COUT_HDR_FL__ << __PRETTY_FUNCTION__
            << "registering to buffer!" << std::endl;
  registerToBuffer();
}
...
bool ots::DataGenReceiver::getNext_(artdaq::FragmentPtrs &output) {
  ...
  if (read<std::string, std::map<std::string, std::string>>(buffer_) < 0)
    ...
    // std::cout << __COUT_HDR_FL__ << __PRETTY_FUNCTION__ << name_ << " Buffer:
    // " << buffer << std::endl;
    unsigned long long value;
    memcpy((void *)&value, (void *)buffer_.substr(2).data(),
           8); // make data counter
    std::cout << __COUT_HDR_FL__ << __PRETTY_FUNCTION__ << std::hex << value
              << std::dec << std::endl;
    ProcessData_(output);
  }
  return true;
...
void ots::DataGenReceiver::ProcessData_(artdaq::FragmentPtrs &frags) {
  ...
  // std::endl;
  if (rawOutput_) {
    std::string outputPath = rawPath_ + "/DataGenReceiver-" + ip_ + ":" +
  ...
  frags.emplace_back(artdaq::Fragment::FragmentBytes(
      initial_payload_size, ev_counter(), fragment_id(),
  ...
  std::cout << __COUT_HDR_FL__
            << "DJN2 after, datasizebytes=" << thisFrag.asksize() << std::endl
            << std::endl;

  memcpy(thisFrag.dataBegin(), (&buffer_[2]),
  ...
  //	}
  TLOG(TLVL_INFO, "DataGenReceiver")
      << "Done with DataGenReceiver Packet Processing Loop. Deleting "
         "PacketBuffers"
      << std::endl;
