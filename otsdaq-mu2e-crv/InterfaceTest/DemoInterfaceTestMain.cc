...
int main(int argc, char **argv) {
  ...
  std::pair<std::string, ConfigurationGroupKey> theGroup(
      configurationGroupName, ConfigurationGroupKey(configurationKeyValue_));

  theConfigurationManager_->loadConfigurationGroup(theGroup.first,
                                                   theGroup.second, true);

  FEOtsUDPTemplateInterface *theInterface_ = new FEOtsUDPTemplateInterface(
      interfaceUID_, theXDAQContextConfigTree_,
      supervisorConfigurationPath_ + "/LinkToFEInterfaceTable/" +
          interfaceUID_ + "/LinkToFETypeConfiguration");
