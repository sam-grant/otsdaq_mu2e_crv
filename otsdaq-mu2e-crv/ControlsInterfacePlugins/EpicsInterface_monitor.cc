...
#define PV_FILE_NAME                                                           \
  std::string(getenv("SERVICE_DATA_PATH")) +                                   \
      "/SlowControlsDashboardData/pv_list.dat";
...
void EpicsInterface::getControlValues(std::string pvName) {
  if (true) {
    std::cout << "EpicsInterface::getControlValues(" << pvName << ")"
              << std::endl;
  }
  if (!checkIfPVExists(pvName)) {
    std::cout << pvName << " doesn't exist!" << std::endl;
  ...
  SEVCHK(ca_array_get_callback(DBR_CTRL_CHAR, 0,
...
void EpicsInterface::subscribeToChannel(std::string pvName,
...
void EpicsInterface::writePVControlValueToRecord(std::string pvName,
  ...
  if (true) {
    ...
    std::cout << "upper warning limit: " << pdata->upper_warning_limit
              << std::endl;
    std::cout << "lower warning limit: " << pdata->lower_warning_limit
              << std::endl;
    std::cout << "lower alarm limit: " << pdata->lower_alarm_limit << std::endl;
    std::cout << "upper control limit: " << pdata->upper_ctrl_limit
              << std::endl;
    std::cout << "lower control limit: " << pdata->lower_ctrl_limit
              << std::endl;
...
void EpicsInterface::writePVValueToRecord(std::string pvName,
                                          std::string pdata) {
...
void EpicsInterface::writePVAlertToQueue(std::string pvName, const char *status,
                                         const char *severity) {
  if (!checkIfPVExists(pvName)) {
    std::cout << pvName << " doesn't exist!" << std::endl;
    return;
  ...
  if (mapOfPVInfo_.find(pvName) != mapOfPVInfo_.end()) {
    if (mapOfPVInfo_.find(pvName)->second != NULL) // Check to see if the pvName
                ...
                // subscription
      {
        ...
        std::string units, upperDisplayLimit, lowerDisplayLimit,
