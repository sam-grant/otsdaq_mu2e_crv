...
void EpicsInterface::subscribeJSON (std::string pvList)
{
	...
	if (pvList.find (JSON) != std::string::npos)
	{
		...
		do
		{
			...
			{
				std::cout << pvName << " not found in file! Not subscribing!" << std::endl;
			}
...
void EpicsInterface::eventCallback (struct event_handler_args eha)
{
	...
	if (eha.status == ECA_NORMAL) {
		...
		case DBR_CTRL_CHAR:
			...
			((EpicsInterface *)eha.usr)->writePVControlValueToRecord (ca_name (eha.chid), ((struct dbr_ctrl_char *)eha.dbr));  //write the PV's control values to records
			break;
		case DBF_DOUBLE:
			if (DEBUG) {
...
{
	if (DEBUG) {
		std::cout << "EpicsInterface::checkIfPVExists(): PV Info Map Length is " << mapOfPVInfo_.size () << std::endl;
	}
...
void EpicsInterface::loadListOfPVs ()
{
	...
	//get parameters (e.g. HIHI("upper alarm") HI("upper warning") LOLO("lower alarm")) for each pv
	for (auto pv : mapOfPVInfo_)
	{
...
void EpicsInterface::getControlValues (std::string pvName)
{
	...
	SEVCHK (ca_array_get_callback (DBR_CTRL_CHAR, 0, mapOfPVInfo_.find (pvName)->second->channelID, eventCallback, this), "ca_array_get_callback");
...
void EpicsInterface::createChannel (std::string pvName)
{
	...
	if (mapOfPVInfo_.find (pvName)->second != NULL)                 // Check to see if the pvName maps to a null pointer so we don't have any errors
		if (mapOfPVInfo_.find (pvName)->second->channelID != NULL)  //channel might exist, subscription doesn't so create a subscription
		{
			...
			{
				if (DEBUG) {
					std::cout << "Channel to " << pvName << " already exists!" << std::endl;
				}
				return;
			}
			if (DEBUG) {
				std::cout << "Channel to " << pvName << " exists, but is not connected! Destroying current channel." << std::endl;
			}
...
void EpicsInterface::subscribeToChannel (std::string pvName, chtype subscriptionType)
{
	...
	if (DEBUG) {
		std::cout << "Trying to subscribe to " << pvName << ":" << mapOfPVInfo_.find (pvName)->second->channelID << std::endl;
	}
	...
	if (DEBUG) {
		std::cout << "EpicsInterface::subscribeToChannel: Created Subscription to " << mapOfPVInfo_.find (pvName)->first << "!\n"
		          << std::endl;
	}
...
void EpicsInterface::writePVControlValueToRecord (std::string pvName, struct dbr_ctrl_char *pdata)
{
	if (DEBUG) {
...
void EpicsInterface::debugConsole (std::string pvName)
{
	...
	std::cout << "==============================================================================" << std::endl;
	std::cout << "Status:     "
	          << " | " << mapOfPVInfo_.find (pvName)->second->alerts.size () << " | " << mapOfPVInfo_.find (pvName)->second->alerts.front ().status << std::endl;
	std::cout << "Severity:   "
	          << " | " << mapOfPVInfo_.find (pvName)->second->alerts.size () << " | " << mapOfPVInfo_.find (pvName)->second->alerts.front ().severity << std::endl;
	std::cout << "==============================================================================" << std::endl;
	...
	if (mapOfPVInfo_.find (pvName) != mapOfPVInfo_.end ())
	{
		if (mapOfPVInfo_.find (pvName)->second != NULL)                 // Check to see if the pvName maps to a null pointer so we don't have any errors
			if (mapOfPVInfo_.find (pvName)->second->channelID != NULL)  //channel might exist, subscription doesn't so create a subscription
			{
				...
				std::string    units, upperDisplayLimit, lowerDisplayLimit, upperAlarmLimit, upperWarningLimit, lowerWarningLimit, lowerAlarmLimit, upperControlLimit, lowerControlLimit;
				...
				//					sprintf(&lowerDisplayLimit[0],"%u",set->lower_disp_limit );
				//					sprintf(  &upperAlarmLimit[0],"%u",set->upper_alarm_limit  );
				//					sprintf(&upperWarningLimit[0],"%u",set->upper_warning_limit);
				//					sprintf(&lowerWarningLimit[0],"%u",set->lower_warning_limit);
				//					sprintf(  &lowerAlarmLimit[0],"%u",set->lower_alarm_limit  );
				//					sprintf(&upperControlLimit[0],"%u",set->upper_ctrl_limit);
				//					sprintf(&lowerControlLimit[0],"%u",set->lower_ctrl_limit);

				//					std::string units             = set->units;
				//					std::string upperDisplayLimit (reinterpret_cast<char*>(set->upper_disp_limit   ));
				//					std::string lowerDisplayLimit (reinterpret_cast<char*>(set->lower_disp_limit   ));
				//					std::string upperAlarmLimit   (reinterpret_cast<char*>(set->upper_alarm_limit  ));
				//					std::string upperWarningLimit (reinterpret_cast<char*>(set->upper_warning_limit));
				//					std::string lowerWarningLimit (reinterpret_cast<char*>(set->lower_warning_limit));
				//					std::string lowerAlarmLimit   (reinterpret_cast<char*>(set->lower_alarm_limit  ));
				//					std::string upperControlLimit (reinterpret_cast<char*>(set->upper_ctrl_limit   ));
				//					std::string lowerControlLimit (reinterpret_cast<char*>(set->lower_ctrl_limit   ));
				if (DEBUG)
				{
					std::cout << "Units              :    " << units << std::endl;
					std::cout << "Upper Display Limit:    " << upperDisplayLimit << std::endl;
					std::cout << "Lower Display Limit:    " << lowerDisplayLimit << std::endl;
					std::cout << "Upper Alarm Limit  :    " << upperAlarmLimit << std::endl;
					std::cout << "Upper Warning Limit:    " << upperWarningLimit << std::endl;
					std::cout << "Lower Warning Limit:    " << lowerWarningLimit << std::endl;
					std::cout << "Lower Alarm Limit  :    " << lowerAlarmLimit << std::endl;
					std::cout << "Upper Control Limit:    " << upperControlLimit << std::endl;
					std::cout << "Lower Control Limit:    " << lowerControlLimit << std::endl;
				}
			}

		std::array<std::string, 9> s = {"DC'd", "DC'd", "DC'd", "DC'd", "DC'd", "DC'd", "DC'd", "DC'd", "DC'd"};

		//std::array<std::string, 9> s = {units, upperDisplayLimit, lowerDisplayLimit, upperAlarmLimit, upperWarningLimit, lowerWarningLimit, lowerAlarmLimit, upperControlLimit, lowerControlLimit};
