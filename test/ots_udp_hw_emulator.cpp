...
int main (int argc, char **argv)
{
	...
	//addressSpaceSS << "\t 0x0000000100000007 \t W \t 'Destination MAC address ignored'\n";
	...
	while (1)
	{
		...
		if (FD_ISSET (sockfd, &readfds))
		{
			...
			while (handlerIndex < totalNumberOfBytes &&
			       (numberOfBytes = (buff[handlerIndex + 0] ? 18 : 10)) &&  //assume write is 18 and read is 10
			       handlerIndex + numberOfBytes <= totalNumberOfBytes)
			{
				         ...
				         buff[handlerIndex + 0] == 1)                            //write
				{
					...
					__COUT__ << std::hex << ":::"
					         << "ERROR: The formatting of the string received is wrong! Number of bytes: " << numberOfBytes << " Read/Write " << buff[handlerIndex + 0] << std::endl;
		...
		else
		{
			...
			//__COUT__ << "[" << __LINE__ << "]Is burst enabled? " << (int)dataEnabled << endl;
			if ((int)dataEnabled)
			{
				//generate data
				//__COUT__ << "[" << __LINE__ << "]Count? " << count << " rate: " << data_gen_rate << " counter: " << data_gen_cnt << endl;
				    ...
				    data_gen_cnt != 0)             //still packets to send
				{
					...
					//memcpy((void *)&buff[TX_DATA_OFFSET],(void *)&data_gen_cnt,8); //make data counter
					...
					//	    			memcpy((void *)&value, (void *) data.substr(2).data(),8); //make data counter
					//	    			__COUT__ << value << std::endl;

					if ((numberOfBytes = sendto (sendSockfd, buff, packetSz, 0, p->ai_addr, p->ai_addrlen)) == -1)
					{
						perror ("Hw: sendto");
						exit (1);
					}
