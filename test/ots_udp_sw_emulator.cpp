...
//
// Protocol is specified https://docs.google.com/document/d/1i3Z07n8Jq78NwgUFdjAv2sLGhH4rWjHeYEScAWBzSyw/edit?usp=sharing
//
//compile with:
...
int main (int argc, char *argv[])
{
	...
	case 1:
		...
		for (int i = 0; i < buff[1]; ++i, ++val)
			memcpy ((void *)&buff[RX_DATA_OFFSET + i * 8], (void *)&val, 8);  //increment each time
		packetSz = RX_DATA_OFFSET + buff[1] * 8;

		if ((numbytes = sendto (sockfd, buff, packetSz, 0, p->ai_addr, p->ai_addrlen)) == -1) {
			perror ("sw: sendto");
			exit (1);
		}
